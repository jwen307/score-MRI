...
from datasets.fastmri_dataset import ConditionalSliceDataset
...
class FastMRIDataModule(pl.LightningDataModule):
    ...
    def __init__(self, base_path, center_frac = 0.08, accel_rate = 4, img_size = 320, batch_size:int = 4, num_data_loader_workers:int = 8):
        """
    ...
    def setup(self, stage:str = None):
        ...
        train_path = '{0}_{1}_size{2}_centerfrac{3}_accel{4}'.format('singlecoil','train',self.img_size, self.center_frac, self.accel_rate)
        val_path = '{0}_{1}_size{2}_centerfrac{3}_accel{4}'.format('singlecoil','val',self.img_size, self.center_frac, self.accel_rate)
        ...
        if stage == 'fit' or stage is None:
            self.fastmri_train = ConditionalSliceDataset(os.path.join(self.base_path, train_path))
            
            self.fastmri_val = ConditionalSliceDataset(os.path.join(self.base_path, val_path))
