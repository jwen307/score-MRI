...
class ConditionalDataTransformVol:
    ...
    def __call__(self, kspace: np.ndarray, mask: np.ndarray, target: np.ndarray, attrs: Dict, fname:str, slice_num:int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, str, int, float]:
        ...
        gt_patches = gt_image.unfold(0,32,32).unfold(1,32,32).reshape(-1,2,32,32)

        #Take the Fourier transform to get the k-space of the cropped image
        cropped_kspace = fastmri.fft2c(gt_patches.permute(0,2,3,1))
        ...
        # Get the target
        target_mag = fastmri.complex_abs(gt_patches.permute(0,2,3,1))

        target_mag= target_mag.reshape(-1, gt_patches.shape[-2], gt_patches.shape[-2])
        ...
        return zf_img, gt_patches, masked_kspace, mask, zf_img_mag, target_mag, fname, slice_num, acquisition
...
if vol:
    if not use_complex:
        new_dir = os.path.join(base_dir, '{0}_{1}_size{2}_centerfrac{3}_accel{4}_vol_patches'.format(challenge,dataset_type,img_size, center_frac, accel_rate))
    else:
        new_dir = os.path.join(base_dir, '{0}_{1}_size{2}_centerfrac{3}_accel{4}_complex_vol_patches'.format(challenge,dataset_type,img_size, center_frac, accel_rate))
else:
    if not use_complex:
        new_dir = os.path.join(base_dir, '{0}_{1}_size{2}_centerfrac{3}_accel{4}_patches'.format(challenge,dataset_type,img_size, center_frac, accel_rate))
    else:
        new_dir = os.path.join(base_dir, '{0}_{1}_size{2}_centerfrac{3}_accel{4}_complex_patches'.format(challenge,dataset_type,img_size, center_frac, accel_rate))
...
if __name__ == '__main__':
    ...
    for fname in sorted(files):
        ...
        with h5py.File(fname, 'r') as hf:
            ...
            slice_nums_np = np.asarray(slice_nums).repeat(25)
            ...
            if normstd == 'normalize':
                ...
                std0 = np.std(zf_imgs_np[:,0,:,:])
                std1 = np.std(zf_imgs_np[:,1,:,:])
                mean0 = np.mean(zf_imgs_np[:,0,:,:])
                mean1 = np.mean(zf_imgs_np[:,1,:,:])
                zf_imgs_np[:,0,:,:] = (zf_imgs_np[:,0,:,:] - mean0)/std0
                zf_imgs_np[:,1,:,:] = (zf_imgs_np[:,1,:,:] - mean1)/std1
                zf_mags_np = (zf_mags_np - np.mean(zf_mags_np)) / np.std(zf_mags_np)
                
                target_imgs_np[:,0,:,:] = (target_imgs_np[:,0,:,:]- mean0)/std0
                target_imgs_np[:,1,:,:] = (target_imgs_np[:,1,:,:]- mean1)/std1
                target_mags_np = (target_mags_np- np.mean(zf_mags_np))/ np.std(zf_mags_np)
            ...
            with h5py.File(os.path.join(new_dir, fname.name), 'w') as nf:
                ...
                nf.attrs['normstdval'] = max_val if (normstd == 'normalize') else [std0,std1,mean0,mean1]
                
                #nf.attrs['normstdmean'] = max_val if (normstd == 'normalize') else [mean0,mean1]
